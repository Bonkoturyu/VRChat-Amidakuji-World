...
- [ ] **20人スケール対応**
  ...
  - 横線生成確率の再チューニング
- [ ] **観戦補助機能**
  - あみだくじ床の両脇に、観戦者がカートへ追いつくための加速ゾーンを配置
  - VRChat標準カメラを飛行モードで特定位置へ入れると、固定の俯瞰視点が映る撮影スポットを配置
  - プレイヤー本体の移動・テレポートや、独自の可動カメラギミックは実装しない
  - 俯瞰映像のRenderTexture品質はクライアントごとのローカル設定とし、PCは高解像度、Quest / iOSは低解像度を既定にする
  - 設定画面に俯瞰カメラ品質(OFF / 低 / 中 / 高)を追加し、混在インスタンスでも各端末が独立して選べるようにする
  - **⚠ 起動トリガーの制約**: VRChat 標準カメラ(Photo Camera)の有効状態・トランスフォームを Udon から安定取得する公開 API は無い(動的生成オブジェクトの `GameObject.Find` 依存は SDK 更新で壊れるため非推奨)。したがって**標準カメラの検出に依存せず**、プレイヤーが撮影スポットの判定コライダー内にいる間、または設定 UI のトグルを ON にした間だけ起動する設計とする
  - **省負荷方針として採用**: 上記トリガー成立中(撮影スポット内 or UI トグル ON)だけ、ローカルで俯瞰用 Camera と RenderTexture 更新を有効化
  - プレイヤーが撮影スポットから離れた時、または設定トグルを OFF にした時は、俯瞰用 Camera を無効化して追加描画を停止
  - 大型観戦ディスプレイを主役にはせず、床を走って追いかける既存体験を補助する位置づけにする
- [ ] **カート速度設定**
  - エントリーエリアの設定画面から、次のレースのカート速度を変更可能にする
  - Master のみ変更可能とし、全クライアントへ同期
  - 速度プリセットと許容範囲を決め、VR酔い・追走しやすさ・レース時間を実機確認
  - レース中の変更は禁止し、Idle時の設定をレース開始時に確定
- [ ] **動画プレイヤー追加(待機・交流向けMVP)**
  - v1.0 の「Video Player 不使用」方針を再評価し、採用コンポーネント・対応URL・クロスプラットフォーム制約を短い技術検証で確定
  - エントリーエリアから見やすく、レース観戦や操作パネルを遮らない位置にスクリーンを配置
  - URL入力・再生/停止・シーク・音量の基本操作を用意し、操作権限(全員 / Masterのみ)を決定
  - 再生URL・再生位置・再生状態を参加者間で同期し、Late Joiner の追従を確認
  - PC / Quest / iOS で対応URL・映像表示・音声再生の差異を実機確認
  - 既存BGMとの競合を避けるため、動画再生中のBGM音量低下またはミュート方針を実装
  - Quest実機でFPS・メモリ・ワールド容量への影響を測定し、必要なら画面サイズ・解像度・同時再生数を制限
...
- [ ] **賞品エリアの演出拡張**(v1.0 で爆発・紙吹雪のみ実装済、[ADR-0012](./docs/adr/0012-goal-effect-randomized.md))
  ...
  - ゴールごとに異なる固定装飾・ギミック
  - Aモードでは、先に到着した参加者へ「みんなが到着するまで待ってね！」と案内表示
...
> **観戦者向け機能の評価軸**(2026-06-24 確認): このワールドの観戦体験の本質は「カートの周りを走って追いかける」こと(追いかけ式観戦 [ADR-0009](./docs/adr/0009-follow-alongside-spectator.md))。走行追走に観戦者の手・視線・注意が占有されるため、**走行中に能動操作(エモート発火等)を要求する機能は観戦体験と競合するので採用しない**。観戦者向けアイデアは「走りながら成立する/レース前後の静止時間に完結する」ものに限る。
>
> - 例(可): **当たり予想モード** はレース開始**前**の静止フェーズで投票が完結するため評価軸を満たす(下記)。
> - 例(却下): **応援エモート**(走行中に観戦者が拍手・歓声 SE/パーティクルを撃つ案、2026-06-24)は走行追走中の操作を要求するため不採用。
...
- カートのカスタマイズ(色・形を選べる)
- **走行済みルートの発光表示** — 各カートが通過した縦線・横線をカート色で表示し、結果表示中も経路を確認・撮影できるようにする
- **スタート時の生成演出** — 横線を短時間シャッフル表示してから今回の配置を確定し、カウントダウンへ移る
- 季節イベント装飾 (ハロウィン版あみだくじ等)
- **当たり予想モード(観戦者参加)**(2026-06-24 追加) — レース開始前に観戦者が「どのカートが紙吹雪(当たり)か」を予想ボタンで投票し、結果表示時に的中者名を ResultDisplay に掲示する。当たり配置は `seed` 派生 RNG でレース開始時に決定論的に確定済([ADR-0012](./docs/adr/0012-goal-effect-randomized.md))のため、照合に追加の Sync は不要。**ただし投票の同期設計に注意**: 多数の観戦者が単一オブジェクトの UdonSynced 配列へ同時書込すると Ownership 奪い合いで競合・同期遅延が起きる。回避策は (a) 的中判定を**完全ローカル処理**(各自の画面でのみ自分の的中を表示、同期ゼロ)とするか、(b) 全員の的中者名を掲示する要件なら **Player Object Pool 等で各観戦者が自分の投票オブジェクトを所有**して個別同期する設計を採る。観戦多数 vs 参加 4 人という構成で、観戦者に勝敗の当事者性を与える狙い(運ゲー/パーティ寄りタグ方針と整合)
- **あみだ密度プリセット(疎/標準/密)**(2026-06-24 追加) — Master がエントリーエリア UI で横線生成の重み比を切替え、レース時間と運要素の濃さを調整する。§4.1 / [ADR-0002](./docs/adr/0002-deterministic-rng-seed-sync.md) の重み付き 5 パターン抽選の重み比テーブルを複数用意し、選択制にする。**⚠ 「重み比差替えのみ」では不足**: 現状の決定論レイアウトは全クライアントが同一のハードコード重みで `AmidakujiGenerator.Rebuild(seed)` を実行する前提で、`GameManager` は `seed`/state のみ同期している([ADR-0005](./docs/adr/0005-minimal-sync-variables.md))。プリセットを選択制にするには (1) プリセット番号を**新規 UdonSynced 変数**として追加(`simultaneousFinale` と同様)、(2) `Rebuild` をプリセット(重みテーブル)引数付きに変更、(3) 非 Master・Late Joiner が **`Rebuild` の前に同期済プリセットを読む**ことが必須。これを欠くと同一 seed でも横線・経路・当たり/ハズレがクライアント間で食い違う。v1.1 候補「カート速度設定」とは別軸で相補
- **賞品ピックアップ(記念オブジェクト)**(2026-06-24 追加) — 当たり賞品エリアに VRChat Pickup の物理オブジェクト(トロフィー等)を配置し、持ち帰り・記念撮影に使う。標準 Pickup 機能で実装軽量だが、Quest での同期負荷・持ち出し可能範囲の制限は要検証
