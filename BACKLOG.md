...
- [ ] **観戦補助機能**
  ...
  - **省負荷方針として採用**: Photo Cameraが有効かつ撮影スポットの判定範囲内にある間だけ、俯瞰用CameraとRenderTexture更新を有効化
  - 撮影スポットから標準カメラが離れた時、または標準カメラを閉じた時は、俯瞰用Cameraを無効化して追加描画を停止
...
- **当たり予想モード(観戦者参加)**(2026-06-24 追加) — レース開始前に観戦者が「どのカートが紙吹雪(当たり)か」を予想ボタンで投票し、結果表示時に的中者名を ResultDisplay に掲示する。当たり配置は `seed` 派生 RNG でレース開始時に決定論的に確定済([ADR-0012](./docs/adr/0012-goal-effect-randomized.md))のため、投票 → 結果照合は新規 UdonSynced(投票配列)のみで実装可。観戦多数 vs 参加 4 人という構成で、観戦者に勝敗の当事者性を与える狙い(運ゲー/パーティ寄りタグ方針と整合)
